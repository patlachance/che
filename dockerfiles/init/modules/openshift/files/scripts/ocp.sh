...
deploy_che_to_ocp() {
    ...
    if [ $CHE_OPENSHIFT_GENERATE_SCRIPTS == true ]; then
      ...
      docker run -t --rm -v /var/run/docker.sock:/var/run/docker.sock -v "${CONFIG_DIR}":/data --env-file $HOME/dev/oip/.env -e IMAGE_INIT="$IMAGE_INIT" -e CHE_MULTIUSER="$CHE_MULTIUSER" ${CHE_CLI_IMAGE} config --skip:pull --skip:nightly
